...
if (!isset($_POST['confirm']) || $_POST['confirm'] !== 'yes') {
    echo '<form method="post" style="max-width:800px; margin:20px auto; padding:20px; background-color:#f5f5f5; border:1px solid #ddd;">';
    echo '<h2>Activación de Licencia de RemUI para InteB</h2>';
    echo '<p>Este script realizará las siguientes acciones:</p>';
    echo '<ol>';
    echo '<li>Aplicar sobreescritura de licencia de RemUI</li>';
    echo '<li>Establecer estado de licencia como válido</li>';
    echo '<li>Purgar todas las cachés</li>';
    echo '</ol>';
    echo '<p style="color:#cc0000;"><strong>Advertencia:</strong> Esto solo debe ejecutarse en entornos controlados.</p>';
    echo '<input type="hidden" name="confirm" value="yes">';
    echo '<input type="submit" value="Aplicar licencia y purgar cachés" style="background-color:#4CAF50; color:white; padding:10px 15px; border:none; cursor:pointer;">';
    echo '</form>';
    die();
}

echo '<div style="max-width:800px; margin:20px auto; padding:20px; background-color:#f5f5f5; border:1px solid #ddd;">';
echo '<h2>Aplicando licencia y purgando cachés...</h2>';
...
echo '<p>La licencia de RemUI ha sido sobreescrita y las cachés han sido purgadas.</p>';
echo '</div>';
